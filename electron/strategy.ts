...
interface StrategyState {
  ...
  lastCheck: number;
  checkCount: number;
  marketTrend: 'BULL' | 'BEAR' | 'UNCERTAIN';
  priceChange: number;
  rsiValue: number;
  emaValue: number;
  lastSignal: string;
...
export class TradingStrategy {
  ...
  private async executeStrategy(): Promise<void> {
    try {
      ...
      // 获取多个时间框架的K线数据
      let hourlyKlines: Kline[] = [];
      let fifteenMinKlines: Kline[] = [];
      let fiveMinKlines: Kline[] = [];
      ...
      if (!this.symbolInfo) {
        try {
          this.symbolInfo = await this.withRetry(
            () => marketApi.getSymbolInfo(this.config.symbol), 
            '获取交易对信息'
          );
        } catch (error) {
  ...
  private async updateAccountInfo(): Promise<void> {
    try {
      ...
      // 精确计算USDT余额
      const usdtBalance = this.accountInfo.balances.find((b: any) => b.asset === 'USDT');
      if (usdtBalance) {
        this.state.accountBalance = parseFloat(usdtBalance.free) + parseFloat(usdtBalance.locked);
      } else {
        log.warn('未找到USDT余额信息');
        // 不重置为0，保留上次值
      }
      
      // 计算当前持仓价值
      const baseAsset = this.config.symbol.replace('USDT', '');
      const assetBalance = this.accountInfo.balances.find((b: any) => b.asset === baseAsset);
      if (assetBalance) {
      ...
      // 保留上次的账户信息而不是清零，避免误判
      log.warn('使用上次的账户信息继续操作');
    }
  }

  private async enterPosition(trend: string, currentPrice: number, klines: Kline[]): Promise<void> {
    try {
      const action = trend === 'BULL' ? '买入' : '卖出';
      log.info(`发现入场机会: ${action} ${this.config.symbol}`);
      ...
      if (this.state.accountBalance < requiredBalance) {
        log.error(`余额不足，需要 ${requiredBalance.toFixed(2)} USDT，但账户只有 ${this.state.accountBalance.toFixed(2)} USDT`);
  ...
  private async exitPosition(currentPrice: number): Promise<void> {
    try {
      ...
      log.info(`执行出场: ${action} ${this.config.symbol}`);
  ...
  // 计算所需余额（包含手续费）
  private calculateRequiredBalance(currentPrice: number, trend: string): number {
    const quantity = this.calculateQuantity(currentPrice, trend);
  ...
  // 优化数量计算
  private calculateQuantity(currentPrice: number, trend: string): number {
    if (!this.symbolInfo || !this.state.accountBalance) {
      return 0;
    }
    
    // 获取交易对的精度信息
    const lotSizeFilter = this.symbolInfo.filters.find((f: any) => f.filterType === 'LOT_SIZE');
    if (!lotSizeFilter) {
    ...
    // 应用最小数量限制
    if (quantity < minQty) {
      log.warn(`计算的数量 ${quantity} 小于最小交易数量 ${minQty}`);
      return 0;
    }
  ...
  // 计算平仓数量
  private calculateExitQuantity(currentPrice: number): number {
    if (!this.symbolInfo) {
      return 0;
    }
  ...
  // 优化入场条件
  private shouldEnterPosition(trend: string, priceChange: number, rsi: number, fifteenMinTrend: string): boolean {
    // 趋势不一致时不交易
    if (trend === 'UNCERTAIN') {
      return false;
    }
  ...
  private shouldExitPosition(currentPrice: number): boolean {
    ...
    if (this.state.currentPosition === 'LONG' && currentPrice >= this.state.takeProfitPrice) {
      log.info(`达到止盈价: ${currentPrice} >= ${this.state.takeProfitPrice}`);
      return true;
    }
    ...
    if (this.state.currentPosition === 'LONG' && currentPrice <= this.state.stopLossPrice) {
      log.info(`达到止损价: ${currentPrice} <= ${this.state.stopLossPrice}`);
      return true;
    }
  ...
  // 检查交易量是否在增加
  private checkVolumeIncreasing(): boolean {
    // 实际实现应检查最近K线的交易量趋势
    // 这里简化为返回true
    return true;
  }
  ...
  // 计算动态止盈止损
  private calculateDynamicLevels(currentPrice: number, trend: string, klines: Kline[]): { stopLoss: number, takeProfit: number } {
    // 计算近期波动率
  ...
  // 移动止损调整
  private adjustStopLoss(currentPrice: number): void {
    if (this.state.currentPosition === 'LONG') {
      ...
      if (priceIncrease > profitThreshold) {
        ...
        if (newStopLoss > this.state.stopLossPrice) {
          log.info(`移动止损更新: ${this.state.stopLossPrice.toFixed(8)} → ${newStopLoss.toFixed(8)}`);
  ...
  // 添加订单历史
  private addOrderHistory(order: any) {
  ...
  private async getKlines(interval: string, limit: number): Promise<Kline[]> {
    return marketApi.getKlines(this.config.symbol, interval, limit);
  }

  // 获取账户信息
  private async getAccountInfo() {
    return tradeApi.getAccountInfo(this.config.apiKey, this.config.apiSecret);
  }

  // 带重试的请求
  private async withRetry<T>(fn: () => Promise<T>, label: string, retries = 3, delay = 1000): Promise<T> {
    try {
      return await fn();
    } catch (error) {
      if (retries > 0) {
        log.warn(`获取${label}失败，剩余重试次数: ${retries}`, error);
