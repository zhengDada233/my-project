...
import { StrategyConfig, StrategyState, Kline, OrderExecution } from './strategy-types';
...
export class TradingStrategy {
  ...
  // 获取交易对信息
  private async fetchSymbolInfo(): Promise<void> {
    try {
      const response = await this.publicRequest(`exchangeInfo?symbol=${this.config.symbol}`);
      this.symbolInfo = response.symbols.find((s: any) => s.symbol === this.config.symbol);
      if (!this.symbolInfo) {
  ...
  private addOrderHistory(order: OrderExecution): void {
      ...
      const req = https.get(url, (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            const klines: Kline[] = parsedData.map((k: any[]) => ({
              openTime: k[0],
              open: k[1],
              high: k[2],
              low: k[3],
  ...
  // 检查余额
  private async checkBalance(side: string, quantity: number, price: number): Promise<boolean> {
    try {
      await this.updateAccountInfo();
  ...
  private async sendSignedRequest(params: any): Promise<any> {
    return new Promise((resolve, reject) => {
      try {
