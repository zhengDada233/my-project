...
function initAutoUpdater() {
    ...
    electron_updater_1.autoUpdater.on('update-downloaded', (info) => {
        ...
        Promise.resolve().then(() => __importStar(require('electron'))).then(({ dialog }) => {
...
// 处理签名请求
electron_1.ipcMain.handle('send-signed-request', async (_, params, apiKey, apiSecret) => {
...
electron_1.ipcMain.handle('get-exchange-info', async (event, symbol) => {
    return new Promise((resolve, reject) => {
...
// 添加验证 API 密钥的端点
electron_1.ipcMain.handle('validate-api-credentials', async (event, apiKey, apiSecret) => {
    log.info('=== 验证 API 凭证 ===');
    return new Promise((resolve, reject) => {
        try {
            const params = {
...
electron_1.ipcMain.handle('start-trading-strategy', async (event, config) => {
    try {
        if (!TradingStrategy) {
            try {
...
electron_1.ipcMain.handle('stop-trading-strategy', async (event, symbol) => {
    try {
...
electron_1.ipcMain.handle('get-strategy-status', async (event, symbol) => {
    try {
        const strategy = activeStrategies.get(symbol);
        if (strategy) {
            const status = strategy.getState();
...
// 添加日志IPC处理
electron_1.ipcMain.handle('log-info', (_, message) => {
    log.info('Renderer: ' + message);
});
electron_1.ipcMain.handle('log-error', (_, message) => {
    log.error('Renderer: ' + message);
});
electron_1.ipcMain.handle('log-warn', (_, message) => {
    log.warn('Renderer: ' + message);
});
electron_1.ipcMain.handle('log-debug', (_, message) => {
...
electron_1.ipcMain.handle('get-log-content', async () => {
    try {
...
// 创建订单
electron_1.ipcMain.handle('create-order', async (event, params, apiKey, apiSecret) => {
    log.info('=== 创建订单 ===');
...
// 获取账户信息
electron_1.ipcMain.handle('get-account-info', async (event, apiKey, apiSecret) => {
    log.info('=== 获取账户信息 ===');
    return new Promise((resolve, reject) => {
        try {
            const params = {
...
// 获取K线数据
electron_1.ipcMain.handle('get-klines', async (event, symbol, interval, limit) => {
    log.info(`获取K线数据: ${symbol}, 间隔: ${interval}, 数量: ${limit}`);
    return new Promise((resolve, reject) => {
        const req = https.get(`https://api.binance.com/api/v3/klines?symbol=${symbol}&interval=${interval}&limit=${limit}`, (res) => {
            let data = '';
            res.on('data', chunk => data += chunk);
            res.on('end', () => {
                try {
